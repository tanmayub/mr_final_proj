...
object aa {
    def main(args : Array[String]) : Unit  = {
        ...
        if(args(0).equalsIgnoreCase("--predict")){
            ...
            val model = RandomForestModel.load(sc, args(2) + "/output.tree")
            ...
            var y: RDD[Seq[String]] = sc.textFile(args(1)).map(_.split(","))
            ...
            sc.makeRDD(finalOutToWrite).saveAsTextFile(args(2) + "/predicted_out.csv")
        }
        else {
            ...
            model.save(sc, args(2) + "/output.tree")
